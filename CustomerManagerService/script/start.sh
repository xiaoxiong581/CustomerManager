...
logDir="/var/log/xiaoxiong581/CustomerManager"
if [ ! -d "$logDir" ];then
  mkdir -p $logDir
fi
...
JAVA_OPTS='-XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/opt/software/CustomerManager';
...
java $JAVA_OPTS com.yzx.xiaoxiong581.customermanager.microservice.CustomerManagerMain >> $logDir/out.log 2>&1 &
